...
namespace svg
{
	...
	map<const char*, unsigned char*, ltstr> patterns;
    ...
    void parser::start_element(void* data, const char* el, const char** attr)
    {
		...
		if(strcmp(el, "pattern") == 0) 
		{
			...
			patterns[pat_id] = (unsigned char *)"HEJ HOJ";
			
			unsigned char *test;
			test = patterns[pat_id];
    ...
    bool parser::parse_attr(const char* name, const char* value)
    {
        ...
        if(strcmp(name, "fill") == 0)
        {
			...
			else if(strstr(value, "url"))
			{
				parse_fill_url(value);
			}
            else
            {
	...
	//-------------------------------------------------------------
    void parser::parse_fill_url(const char* str)
    {
		char url[strlen(str)-4];
		int ii=0;
